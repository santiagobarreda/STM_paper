...
Well, part of it is probably how it was written, and where it was published. It’s a very dense, technical paper written by experts for experts and published in a book chapter (but then again, I have plenty of conference proceedings that have higher impact factors than this ground-breaking paper, so there’s more to it than the venue). E.g.: The paper present 7 distinct models, without much handholding for the reader (e.g., no visualization of the core aspects of the model, unlike the figures we provide in our paper). No derivations are provided for any of the models (unlike our paper and SI). No code is provided to run any of the models (unlike the R library that we will distribute with our paper). The bigger picture question, and links to related works beyond vowel perception, receive relatively little space in the paper. None of the consequences for other fields are laid out in the paper (some of that has since been done in other papers).
The results are presented only in passing at the very end of the paper. So, why is nobody using this model? Because they don’t know how to, and they don’t know why they should.

And then there’s the issue that the results presented in NA07 are not the same as ours. In fact, they look a lot weaker than what we present. Why? Because of a tiny little mistake the authors made by evaluating the models in terms of the accuracy of predicting the most common human response, rather than the likelihood of human responses (when we use the same metric as in the original, we replicate the original result). Unfortunately, we only said all of this in passing in a footnote. But arguably, the rather non-decisive original result played a role in limiting the impact of the paper, too.
