...
We believe that @NA07 is one of the most ground-breaking papers in speech perception that almost nobody has ever read. As of the writing of this article, it had been cited less than 40 times (about twice per year), and many of these citations are by the first author. During the same time, there have been 314,000 research articles on vowels. Over 17,000 of these, explicitly mention vowel formants/identification/categorization/perception/recognition, and/or formant perception; over 5,000 additionally mention normalization. Among these articles Lobanov normalization [@lobanov1971] is cited three- to four-times as often as Nearey’s STM or "uniform scaling" [@n78] and about 100-times more often than the PSTM [@NA07]---despite the fact that the latter is the only the only published article that describes *any* incremental inference model for vowel perception in any detail at all. So, here is a promising candidate model of formant/spectral talker-normalization, with far-reaching consequences for research from typology to sociolinguistics to speech perception ... and few seem to know of it. 
...
Why? @NA07 is a technical paper, written by experts for experts. It presents seven different models, without much explanation as to how they are derived, and no code to execute them. Even with access to additional notes kindly provided by the late Terrence Nearey, it took us several days to verify some of the core theoretical results presented in @NA07. We imagine that the model is so underutilized in large parts because nobody knows how to implement it. Here we describe the PSTM, and present a more fully Bayesian extension---the *Bayesian Sliding Template Model (BSTM)*---envisioned in @NA07 but not previously available. We provide derivations of the inference models proposed by Nearey and Assman. We walk through the logic of these models step by step, and provide visualizations of core concepts (e.g,. the different priors and their effect on the relevant posterior distributions). Our supplementary information (SI) contains step-by-step derivations for the most relevant models. We have implemented all models describe here in the R package `STM`, which is designed to be user-friendly and flexible. The present article uses the `STM` package, and is written in Quarto markdown. This markdown document, and all R code it sources, are available as part of the OSF repository at https://osf.io/tpwmv/. This allows interested researchers to re-create all of our analyses with the press of a button in RStudio [@R; @RStudio], and to apply similar analyses to their own data.
...
<!-- I don't know if this needs its own section or even if it should go here. I am putting it here now for organizational purposes -->
<!-- I think it should be integrated either above or into the the next subsection -->
## Biological Plausibility of the PSTM

It is important to note that the model to be outlined is not just a clever model hat best explains human vowel perception compared to competitors, though those things are true. In addition, the PSTM approach to normalization, uniform scaling, is rooted in principled considerations about the biology of auditory perception in the mammalian brain, validated by cross-species comparisons. 

Points to make (potentially):

- uniform scaling is closely related to size varation due to basic physics.
- size perception, aka normalization, is a fundamental aspect of perception in general, and has been observed in all sorts of animal species. 
- size perception, uniform scaling norm, has been compared to perceptual size constancy in vision. 
- the 'traditional' approach to norm is not biologicaly plausible, no magic book of answers is available to listeners
- the PSTM addresses the bootstrap problem of norm: listeners must guess that speaker characteristics together with speech
- as noted by Nusbaum, acoustic to speaker characteristics mapping poses the same many-to-many mapping as acoustics to speech. In other words, guessing both together is HARD, probably explaining why researchers have mosly ignored the problem. 
...
par (mfcol = c(1,3), mar = c(5,.2,.1,.2), oma = c(0.6,9.1,0.2,0.1))
...
mtext (side=1,"Difference in model likelihood\n(compared to BSTM6)",line=4.5)
axis (side=1, cex.axis=1.4, at = c(-1000,0,1000))
...
bmmb::brmplot (tmp, labels = "", ylab = "", xlab = "RMS error in ψ estimation",
               line=FALSE,cex.axis=1.4,cex.lab=1.5,
               xlim = c(0,.15), las = 1, horizontal=FALSE, cex=1.75, col = "grey30")
abline (v=0,lty=3,col="black")
legend("bottomright", legend="(c)", bty="n", cex=1.5)
```

<!-- in the plot above, I dont know if its worth extending the range of the middle panel. what do you think? it would reduce the view of the important differences quite a bit. I provide the mean difference for Lobanov wrt BSTM6 in a paragraph below-->
