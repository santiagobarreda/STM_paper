...
<!--  this whole section is copied from response email. I removed parts that were clearly only for the email-->

We believe the Nearey and Assman 2007 PSTM paper is one of the most ground-breaking papers in speech perception that nobody has ever read. It’s been cited less than 40 times over the last 17 years! To put this into perspective, there have been 98,000 research articles on vowels since 2007. Almost 30,000 of these articles explicitly mention “vowel identification”. Among these articles---since 2007---“Lobanov normalization” is cited 10-times as often as Nearey’s “uniform scaling” (!). A total of three papers mention the incremental procedure introduced in NA07 that we describe in our paper. Notably, the NA07 paper is the only the only published article that describes the incremental inference model in much detail at all---it’s not like all those citations went to another follow-up paper.

So, here is a paper that has the most promising candidate model of formant/spectral talker-normalization, with far-reaching consequences for research from typology to sociolinguistics to speech perception. It’s almost two decades old and few know it, and even fewer understand it. 

Why? How did this happen? Well, part of it is probably how it was written, and where it was published. It’s a very dense, technical paper written by experts for experts and published in a book chapter (but then again, I have plenty of conference proceedings that have higher impact factors than this ground-breaking paper, so there’s more to it than the venue). E.g.: 

* The paper present 7 distinct models, without much handholding for the reader (e.g., no visualization of the core aspects of the model, unlike the figures we provide in our paper). 

* No derivations are provided for any of the models (unlike our paper and SI). No code is provided to run any of the models (unlike the R library that we will distribute with our paper). 
* The bigger picture question, and links to related works beyond vowel perception, receive relatively little space in the paper. 

* None of the consequences for other fields are laid out in the paper (some of that has since been done in other papers).

* The results are presented only in passing at the very end of the paper. 

So, why is nobody using this model? Because they don’t know how to, and they don’t know why they should.And then there’s the issue that the results presented in NA07 are not the same as ours. In fact, they look a lot weaker than what we present. Why? Because of a tiny little mistake the authors made by evaluating the models in terms of the accuracy of predicting the most common human response, rather than the likelihood of human responses (when we use the same metric as in the original, we replicate the original result). Unfortunately, we only said all of this in passing in a footnote. But arguably, the rather non-decisive original result played a role in limiting the impact of the paper, too.
 
What we were trying to do in this short manuscript is to (1) make this idea accessible to others, by sparing them the work we had to do and by making a library that makes it trivial to use the model, and (2) provide a succinct intro that highlights the relevance of the work, while (3) given all due credit to Terrance and Peter. 
...
<!-- first mention of Lobanov, are hints needed earlier??-->
 + **Normalization using the [@lobanov1971] method** using the entire training data. This method involves standardization of each speaker's formants by subtracting the mean and dividing by the standard deviation of each formant, independently for each time point. This is perhaps the most commonly used normalization method in speech research as of this writing. However, there are theoretical [@b20] and empirical [@b21] reasons to think it may not provide an adequate model of human vowel perception.
...
<!-- NEW paragraph about Lobanov normalization-->
The results for Lobanov normalization show an even more extreme divergence between the prediction of listener behavior and intended vowel category. Other than the method 1 single trial approach, Lobanov normalization provides the worst fit to listeners' responses (the mean different to BSTM method 6 is -2964). However, it provides the best fit to the intended vowel category (the mean different to BSTM method 6 is 2378). This is consistent with the idea that Lobanov normalization is a very poor model of human speech perception, even if it excels at predicting intended category labels.
