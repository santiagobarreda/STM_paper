...
<!-- I think this makes more sense here, to get readers more invested. I took a crack at it -->

It is important to note the PSTM is not just a clever model that best explains human vowel perception compared to competitors, though those things are true. The PSTM is rooted in principled considerations about the biology of auditory perception in the mammalian brain, validated by cross-species comparisons. The uniform scaling of spectral patterns is closely related to size variation due to the physics of acoustic resonators. As a result, many animal species have been found to separate uniform scaling (size information) from the communicative information present in the signal. As noted by @b20, when human listeners exhibit this behavior we call it 'normalization'. Furthermore, the PSTM helps address the 'bootstrap' problem of speech normalization: listeners must know some speaker characteristics in order to accurately identify speech, but must accurately identify speech in order to properly normalize. Speaker characteristics and speech acoustics enter into the same many-to-many mapping relationship as the linguistic signal and speech acoustics. Consequently, determining both simultaneously from the speech signal is difficult, and the PSTM is one of the few models that addresses this problem in an empirically testable manner.
