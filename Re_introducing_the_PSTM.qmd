...
Why? Part of the answer might lie in the dense, technical style of the paper. Even experts might struggle to follow the assertions in the paper. No derivations are provided for any of the models (unlike the supplementary information provided here). No code is provided to run any of the models (unlike the R library that we will distribute with our paper). Even with access to additional notes kindly provided by the late Terrence Nearey, it took us several attempts to verify some of the core theoretical results presented in @NA07. We imagine that the model is so underutilized because nobody knows how to implement it. A second reason for the relatively low visibility of the paper might originate in the seemingly less than striking core result. As we show below, these results were caused by problematic choice the authors made during the evaluation of the PSTM against listeners' perception. Once this issue is addressed, it reveals that the incremental inference procedure proposed for the PSTM explains listeners' perception very well, and substantially better than the most frequently used normalization account (Lobanov). 
