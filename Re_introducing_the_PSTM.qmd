...
Of note, the use of likelihood as a metric to assess the A) fit against listeners' perception and B) the intended category differs from the approach in @NA07. Nearey and Assman instead used the accuracy (under the criterion choice rule) of predicting listeners' most common response to a stimulus for A) and XXX for B). <!-- fill in --> This metric is now understood to be problematic, as it does not capture how well a model predicts the *distribution* of responses [for discussion, @persson2024]. Indeed, we find that our choice of a more adequate evaluation metric leads to a much clearer result.
...
We compare 11 different methods that differ in terms of how they estimate $\psi_s$, and whether they estimate $\psi_s$ at all. Some of these were included in @NA07, others were not (incl. the popular Lobanov method):
 ...
 + **Normalization using the most commonly cited normalization method [@lobanov1971] ** 
